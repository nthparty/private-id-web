...
mod tests {
    ...
    fn test_input_to_url() {
        let r = host_into_url("http://foo.bar:3001/path?1234");
        assert_eq!(r.domain().unwrap(), "foo.bar");

        let r2 = host_into_url("http://20.00.10.10:3001/path?1234");
    ...
    fn test_noscheme_to_url() {
        let r = host_into_url("foo.bar:3001/path?1234");
        assert_eq!(r.scheme(), "https");
        assert_eq!(
            host_into_url("http://foo.bar:3001/path?1234").scheme(),
            "http"
        );
        assert_eq!(
            host_into_url("https://foo.bar:3001/path?1234").scheme(),
            "https"
        );
    }
}
